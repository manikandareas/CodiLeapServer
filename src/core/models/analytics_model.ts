...
export const DailyStatsSchema = z.object({
  timeSpent: z
    ...
    .int()
    .min(0)
    .describe("Total time spent studying in minutes"),
  lessonsCompleted: z
    .number()
    .int()
    .min(0)
    .describe("Number of lessons completed"),
...
export const WeeklyTrendItemSchema = z.object({
  ...
  timeSpent: z
    ...
    .int()
    .min(0)
    .describe("Time spent studying on this date"),
  progress: z
    .number()
    .min(0)
    .max(1)
    .describe("Learning progress on a scale of 0 to 1"),
});

export const LearningPatternsSchema = z.object({
  mostProductiveHours: z
    .array(z.number().int().min(0).max(23))
    .describe("Most productive hours of the day"),
  averageSessionDuration: z
    .number()
    .int()
    .min(0)
    .describe("Average study session duration in minutes"),
  chronotype: z
    .enum(["Early Bird", "Moderate", "Night Owl"])
    .describe("User's preferred study time"),
});
